    ...
    while True:
        try:
            ...
            try:
                ...
                # 'image has no palette' 에러가 나면, RGBA로 먼저 변환해 투명도를 해결하고
                # 그 다음에 원하는 모드(L 또는 RGB)로 다시 변환합니다.
                yield img.convert('RGBA').convert(mode)
...
class Dataset(data.Dataset):
    @staticmethod
    def _resolve_field_folder(folder, *names):
        base_folder = Path(folder)
        candidate_roots = [base_folder / 'gifs', base_folder]
        ...
        return candidate_roots[0] / names[0]

    @staticmethod
    def _load_gif_paths(folder, exts):
        paths = [p for ext in exts for p in folder.glob(f'**/*.{ext}')]
        paths = sorted(paths, key=lambda x: int(x.stem))
        assert all([int(p.stem) == i for i, p in enumerate(paths)]), 'file position is not equal to index'
        return paths
    ...
    def __init__(
        ...
        # load topo data
        topo_folder = self._resolve_field_folder(folder, 'topo')
        self.paths_top = self._load_gif_paths(topo_folder, exts)

        if reference_frame == 'lagrangian':
            # load u_1 data
            u_1_folder = self._resolve_field_folder(folder, 'u_1')
            self.paths_u_1 = self._load_gif_paths(u_1_folder, exts)

            assert len(self.paths_u_1) == len(self.paths_top), 'number of files in fields and top folders are not equal.'

            # load u_2 data
            u_2_folder = self._resolve_field_folder(folder, 'u_2')
            self.paths_u_2 = self._load_gif_paths(u_2_folder, exts)
            
            assert len(self.paths_u_2) == len(self.paths_top), 'number of files in fields and top folders are not equal.'
                
        # load mises data
        s_mises_folder = self._resolve_field_folder(folder, 's_mises', 's_mise')
        self.paths_s_mises = self._load_gif_paths(s_mises_folder, exts)

        assert len(self.paths_s_mises) == len(self.paths_top), 'number of files in fields and top folders are not equal.'

        # load s_22 data
        s_22_folder = self._resolve_field_folder(folder, 's_22')
        self.paths_s_22 = self._load_gif_paths(s_22_folder, exts)

        assert len(self.paths_s_22) == len(self.paths_top), 'number of files in fields and top folders are not equal.'

        # load ener data
        ener_folder = self._resolve_field_folder(folder, 'ener')
        self.paths_ener = self._load_gif_paths(ener_folder, exts)

        assert len(self.paths_ener) == len(self.paths_top), 'number of files in fields and top folders are not equal.'

        frame_range_file = base_folder / 'frame_range_data.csv'
        ...
        if reference_frame == 'eulerian':
            ...
            with open(base_folder / 'min_max_values.csv', 'w', newline='') as file:
        ...
        label_file = base_folder / 'stress_strain_data.csv'
    ...
    def __getitem__(self, index):
        ...
        if self.reference_frame == 'eulerian':
            ...
            # IMPORTANT: we must do this after scaling values true range to ensure a 0 value corresponds to true 0 field value
            for i in range(4):
                # 1. 마스크 조건 생성 (0인 부분 찾기)
                mask_condition = (topologies[0, 0, :, :] == 0.)

                # 2. 마스크를 데이터(tensor[i]) 크기인 [11, 96, 96]으로 똑같이 확장
                mask = mask_condition.expand_as(tensor[i])

                # 3. 적용 (이제 모양이 [11, 96, 96]으로 서로 같으므로 에러 없음)
                tensor[i][mask] = 0.
            # normalize to global range
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.load_results_folder = Path(load_results_folder) if load_results_folder is not None else self.results_folder
    ...
    def _load_state_dict_flex(self, target, state_dict, strict=True):
        target_state = target.state_dict()
        target_has_module = self._state_dict_has_module_prefix(target_state)
        source_has_module = self._state_dict_has_module_prefix(state_dict)

        if target_has_module and not source_has_module:
            adapted_state_dict = self._add_module_prefix(state_dict)
        elif not target_has_module and source_has_module:
            adapted_state_dict = self._strip_module_prefix(state_dict)
        else:
            adapted_state_dict = state_dict

        target.load_state_dict(adapted_state_dict, strict=strict)
    ...
    def load(
        ...
        path = str(self.load_results_folder) + '/model/step_' + str(self.step) + '/checkpoint.pt'
        ...
        try:
            ...
            self._load_state_dict_flex(self.model, state_dict, strict = strict)
        except RuntimeError as exc:
            raise RuntimeError(f'Failed loading model state dict from {path}: {exc}') from exc
            
            
        # Optimizer의 state_dict 로드
        try:
            # 만약 다중 GPU에서 저장되었을 경우 불일치 발생 가능
            opt_state_dict = loaded_obj['optimizer']
            self.opt.load_state_dict(opt_state_dict)
        except Exception:
            self.accelerator.print('resuming with new optimizer')

        # EMA 모델의 state_dict 로드
        try:
            ema_state_dict = loaded_obj['ema']
            self._load_state_dict_flex(self.ema_model, ema_state_dict, strict=strict)
        except RuntimeError as exc:
            raise RuntimeError(f'Failed loading EMA model state dict from {path}: {exc}') from exc
